...
noexcept -> Best_fit
{
  ...
  [ frequencies, &alpha_scale, b2min, b2max,b1min, b1max, &L, &RC_scale , b1_i]
  ( const double * x )
  noexcept
  {
    ...
    auto const b1 = b1_i;
    auto const b2 =  quantity< si::dimensionless >( x_limiter2( x[1], b2min, b2max ) ); //  detector radius
