...
noexcept -> Best_fit
{
  ...
  [ frequencies, &alpha_scale, b1min, b1max, &L, &RC_scale ]
  ( const double * x )
  noexcept
  {
    ...
    std::cout <<  alpha << "\t" << b1 *L << "\t" << RC_time_constant <<  "\n" ;
    auto const updated_elements = make_tuple( alpha, b1, RC_time_constant ) ;
...
//      calibrated_observations(observations )
  ...
  ( const double * x ) noexcept
  {
    ...
    frequency_sweep( b1, deltaT, frequencies, L, alpha ) ;
  ...
  auto const b2_dummy = quantity< si::dimensionless >( 42.0  );
  
  auto const result = Best_fit(
    fitted_slab, b2_dummy, b1, frequencies,
