...
(
  ...
  model_phases( model_phases_ ), detector_offset( detector_offset_ )
{}
...
) noexcept -> Best_fit
{
  ...
  auto const e_sub = slab_initial.thermal_effusivity();
  ...
  auto const b3_i = dimensionless::b( detector_offset_distance, L_coat );
  ...
  auto model_parameters = vector< double >
  {
    ...
    kx_limiter1( b3_i.value() )      // detector offset
...
//    x_to_k_constrained_from_0_to_1( hp_initial.R1.value() )      // interface reflectivity
 ...
 //   quantity<si::dimensionless>( k_to_x_constrained_from_0_to_1( x[6] ) );
  ...
  ( const double * x ) noexcept
  {
    ...
    auto const b3_offset = get< 3 >(t);

    
    auto const predictions =
    model::tbc2009::offset_detector::average_surface_phases(
    frequencies, hp, tp, L_coat, substrate.thermal_diffusivity(), b3_offset, b2 );
  ...
  auto const result =
  Best_fit( L_coat, b2, substrate, hp, tp,
            frequencies, model_predictions, b3_offset );
...
) noexcept -> Best_fit
{
  ...
  return from_phases(
    o.first, o.second ,
    slab_initial, slab_substrate, hp_initial,
    detector_view_radius, detector_offset_distance );
}
